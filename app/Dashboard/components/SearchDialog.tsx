...
export function SearchDialog({ open, onOpenChange }: SearchDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] sm:w-[485px] md:w-[485px] lg:w-[485px] h-[70vh] sm:h-[368px] max-w-[95vw] max-h-[90vh] p-0 overflow-hidden fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 [&>button]:top-3 [&>button]:right-3 sm:[&>button]:top-4 sm:[&>button]:right-4 rounded-none">
        ...
        <div className="space-y-0 h-full flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-2 mt-3 sm:gap-3 border-b px-3 sm:px-4 py-2 sm:py-2.5 focus-within:border-gray-400 transition-colors flex-shrink-0">
            <CiSearch className="h-4 w-4 sm:h-5 sm:w-5 text-gray-500 flex-shrink-0" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Search..."
              className="flex-1 text-xs sm:text-sm outline-none bg-transparent text-gray-900 placeholder:text-gray-400"
              autoFocus
            />
          </div>
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide space-y-3 sm:space-y-4 py-3 sm:py-4">
            ...
            <>
              ...
              {(filteredCategories.length > 0 || !hasSearchQuery) && (
                <div className="space-y-1.5 sm:space-y-2 px-3 sm:px-4">
                  <h3 className="text-[10px] sm:text-xs font-semibold text-[#71717A] tracking-wide uppercase">Categories</h3>
              ...
              {(filteredFiles.length > 0 || !hasSearchQuery) && (
                <div className="space-y-1.5 sm:space-y-2 px-3 sm:px-4">
                  <h3 className="text-[10px] sm:text-xs font-semibold text-[#71717A] tracking-wide uppercase">Files</h3>
                  <div className="space-y-0.5 sm:space-y-1">
            ...
            <div className="flex flex-col items-center justify-center h-32 sm:h-40 text-gray-500 px-4">
              <p className="text-xs sm:text-sm text-black">No results found.</p>
              <p className="text-[10px] sm:text-xs text-gray-500 mt-1">Try different keywords</p>
            </div>
          )}
          </div>
        </div>
